...
/* Modify */
void
checkThread(struct thread *t, void *aux UNUSED)
{
  if (t->status == THREAD_BLOCKED && t->blocking_time > 0)
  {
    --t->blocking_time;
	if (t->blocking_time == 0)
	  thread_unblock(t);
  }
}
/***/
...
/* Modify */
/* So sanh muc do uu tien cua thread */
bool
...
thread_hold_lock(struct lock *lock)
{
  ...
  list_insert_ordered(&cur->list_locks_holding, &lock->elem, lock_cmp_priority, NULL);
...
thread_update_priority(struct thread *t)
{
  ...
  if (!list_empty(&t->list_locks_holding))
  {
    list_sort(&t->list_locks_holding, lock_cmp_priority, NULL);
	lock_pri = list_entry(list_front(&t->list_locks_holding), struct lock, elem)->max_priority;
    if (max_pri < lock_pri)
	  max_pri = lock_pri;
  }
  t->priority = max_pri;
...
thread_set_priority (int new_priority) 
{
  ...
  */
  ...
  if (list_empty(&cur->list_locks_holding) || new_priority > old_pri)
  {
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  list_init(&t->list_locks_holding);
  t->lock_waiting = NULL;
  t->nice = 0;
 /***/
