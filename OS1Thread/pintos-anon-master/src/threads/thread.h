...
struct thread
  {
	...
	/* Modify */
	int64_t blocking_time;              /* Thoi gian thread bi block */
	int base_priority;                  /* Used to record thread's priority when it's not being donated. */
	struct list list_locks_holding;          /* List of locks the thread is holding. */
...
/* Modify */
void checkThread(struct thread *t, void *aux UNUSED);
/***/
